...
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/shalangbaizicu/ui/main/%E5%BF%8Dware.lua"))()
local win = library:new("小表弟外挂")
--
...
--
local RZCQ1 = win:Tab("小表弟者传奇",'6035145364')
--
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local UITab = UITab1:section("『☯小表弟外挂公告☯』",true)
--
...
--
local RZCQ = RZCQ1:section("小表弟者传奇",false)
...
UITab:Label("我是小表弟这是我写的外挂,大表弟是臭咸鱼")
...
_b.Size=UDim2.new(0,304,0,21)_b.Font=Enum.Font.Arial;_b.Text="小表弟外挂"
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      DoorsTab:Button(" 小表弟  -- Doors", function()
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RZCQ:Button("小表弟者传奇", function()
    loadstring(game:HttpGet('https://pastebin.com/raw/bGSJHMg5'))();
end)

RZCQ:Button("小表弟者传奇(英文)", function()
...
local function donate(plr, amount)
    ...
    notify('小表弟', '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
function autobuyv2(o)
    ...
    if item == nil then
        notify('小表弟', "没有找到商品或者没有刷新，请你等待", 4)
        repeat
            task.wait()
            item = finditem(o)
        until item ~= nil
...
Section:Button("获取当前死亡后加载时间", function()
    bailib:Notification('小表弟', " 时间为" .. bai.loaddupeaxewaittime .. "秒", "好")
end)
...
Sectionhuanjin:Button("船离开时间", function()
    ...
    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        ...
        notify('小表弟', "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("桥上升时间", function()
    ...
    if game.Workspace.Bridge.DownTime.Value < 0 then
        ...
        notify('小表弟', "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("查看游戏时间", function()
    notify('小表弟', "游戏时间为" .. tostring(game.Lighting.TimeOfDay):sub(1, -2):sub(1, -2):sub(1, -2), 4)
end)
...
FM:Button("bark2.0", function()
 ...
 FM:Button(" 小表弟  -- 伐木大亨2(盗版浮光掠影)", function()
...
if REN["白名单"]["开发者白名单"]["状态"] == true then
        ...
        if REN["限免"] == true then
            notify(" 小表弟 ","现在是限免时间，无需白名单",3)
            wait(1)
            notify(" 小表弟 ","正在启动脚本....",5)
            wait(3)       
            scripts()
        else
        if R == "小表弟脚本交流群:139341298" then
            notify(" 小表弟 ","保护函数正确",3)
        else   
